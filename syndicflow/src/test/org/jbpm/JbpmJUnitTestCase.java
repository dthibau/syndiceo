...
public abstract class JbpmJUnitTestCase extends TestCase {
	...
	public void assertNodeActive(long processInstanceId, StatefulKnowledgeSession ksession, String... name) {
		...
		if (processInstance instanceof WorkflowProcessInstance) {
			assertNodeActive((WorkflowProcessInstance) processInstance, names,ksession);
		}
		if (!names.isEmpty()) {
	...
	private void assertNodeActive(NodeInstanceContainer container, List<String> names, StatefulKnowledgeSession ksession) {
		for (NodeInstance nodeInstance: container.getNodeInstances()) {
			...
			if (nodeInstance instanceof NodeInstanceContainer) {
				assertNodeActive((NodeInstanceContainer) nodeInstance, names, ksession);
			}
			if ( nodeInstance instanceof SubProcessNodeInstance ) {
				WorkflowProcessInstance subProcessInstance = (WorkflowProcessInstance)ksession.getProcessInstance(((SubProcessNodeInstance) nodeInstance).getProcessInstanceId());
				assertNodeActive(subProcessInstance,names,ksession);
			}
		}
	}
